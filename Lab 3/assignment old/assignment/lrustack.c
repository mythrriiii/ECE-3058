...
lru_stack_t* init_lru_stack(int size) {
    ...
    // Allocate memory for lru_indices array
    stack->lru_indices = (int *)malloc(stack->size * sizeof(int));

    // Initialize lru_indices array with indices from 0 to (size - 1)
    for (int i = 0; i < stack->size; i++) {
        stack->lru_indices[i] = i;
    }

    // Instantiate the indices array and set all the values
...
int lru_stack_get_lru(lru_stack_t* stack) {
    ...
    int lru_index = (stack->size) - 1;
    return stack->lru_indices[lru_index];
...
void lru_stack_set_mru(lru_stack_t* stack, int n) {
    ...
    for (int i = 0; i < (stack->size); i++) {
        ...
        // find MRU candidate
        if (stack->lru_indices[i] == n) {
            mru_index = i;
        }

    }

    // change the stack accordingly
    for (int i = mru_index; i > 0; i--) {
        stack->lru_indices[i] = stack->lru_indices[i - 1];
    }

    //place mru at the start
    stack->lru_indices[0] = n;
