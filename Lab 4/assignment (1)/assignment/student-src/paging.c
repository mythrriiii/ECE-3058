...
void system_init(void) {
    ...
    //set frame_table pointer to mem
    frame_table = (fte_t*)mem; 

    // set all the memory bits to 0
    for(int i = 0; i < NUM_FRAMES; i++) 
    {
        //set everything for that entry in memory to 0
    ...
    //set first element in the table to be protected
...
void proc_init(pcb_t *proc) {
    ...
    //get a free frame to zero out
    pfn_t empty_pfn = free_frame();
    //get pointer to that frame
    uint8_t* empty_pfn_pointer = (uint8_t*) (mem + (empty_pfn * PAGE_SIZE));
    //set everything for that entry in memory to 0
    memset(empty_pfn_pointer, 0, PAGE_SIZE);
    ...
    //update process with new frame number
    proc->saved_ptbr = empty_pfn;
    //make it protected and set the process
    frame_table[empty_pfn].protected = 1;
    frame_table[empty_pfn].process = proc;
}
...
void context_switch(pcb_t *proc) {
    ...
    //change the process to the saved one
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    // get pointer to required entry
    pte_t* pg_table_entry = (pte_t*)(mem + (PTBR * PAGE_SIZE)) + virtual_pn;

    /* If an entry is invalid, just page fault to allocate a page for the page table. */

    if (!pg_table_entry->valid) {
        page_fault(address);
    }

    /* Set the "referenced" bit to reduce the page's likelihood of eviction */
    frame_table[pg_table_entry->pfn].referenced = 1;
    ...
    */
    ...
    uint8_t *physical_address = (mem + ((pg_table_entry->pfn) * PAGE_SIZE) + offset);
...
void proc_cleanup(pcb_t *proc) {
    ...
    pfn_t other_pfn = proc->saved_ptbr;   // gets a different frame table number
    frame_table[other_pfn].protected = 0; // free by setting protected bit to 0
}
